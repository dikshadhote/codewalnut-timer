...
interface TimerModalProps {
  isOpen: boolean;
  onClose: () => void;
  timer?: Timer; // Optional, for editing purpose
  isEditMode: boolean; // Flag to differentiate Add vs Edit modes
}
...
export const TimerModal: React.FC<TimerModalProps> = ({
  ...
  const [title, setTitle] = useState(isEditMode && timer ? timer.title : '');
  const [description, setDescription] = useState(isEditMode && timer ? timer.description : '');
  const [hours, setHours] = useState(isEditMode && timer ? Math.floor(timer.duration / 3600) : 0);
  const [minutes, setMinutes] = useState(isEditMode && timer ? Math.floor((timer.duration % 3600) / 60) : 0);
  const [seconds, setSeconds] = useState(isEditMode && timer ? timer.duration % 60 : 0);
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <ButtonGroup
            ...
            submitText={isEditMode ? 'Save Changes' : 'Start Timer'}
          />
        </form>
      </div>
    </div>
